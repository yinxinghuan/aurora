...
Aurora 是一个 `casual` 类型的移动端小游戏：极光 — 触摸夜空，柔光帘随手指流淌，三种夜色：极地翠绿、暮粉霞紫、午夜深蓝

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #06081a、#0d1530、rgba(255,255,255,0.78)、rgba(255,255,255,0.06)、#000、rgba(0, 0, 0, 0.22)、rgba(255, 255, 255, 0.08)、rgba(0, 0, 0, 0.18)；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 -apple-system, "SF Pro Display", "Helvetica Neue", system-ui, sans-serif，按钮与状态文字保持 12-24 px 的可读范围。
...
- `VOICE_CAP`：5
- `MAX_RIBBONS`：7
- `IDLE_SPAWN_INTERVAL`：1500
- `TARGET_IDLE_RIBBONS`：5
- `IDLE_BAND_COUNT`：5
- `FRAME_MS`：1000
- `SUB`：10
